...
function roll(type) {
    ...
    TS.debug.log("Created roll with ID: " + diceSetResponse);
}
...
async function handleRollResult(rollEvent) {
    if (trackedIds[rollEvent.payload.rollId] == undefined) {
        ...
        TS.debug.log("Ignoring roll with ID: " + rollEvent.payload.rollId);
        return;
    }
    TS.debug.log("Handling roll with ID: " + rollEvent.payload.rollId);
    if (rollEvent.kind == "rollResults") {
        ...
        TS.debug.log("Final result for roll ID " + roll.rollId + " is: " + finalResult);
